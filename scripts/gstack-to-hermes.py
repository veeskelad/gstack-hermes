...
HERMES_NOTE = (
    ...
    "> 1. **Standalone headers / paragraph titles** — `**bold**` is OK and renders "
    "correctly via `format_message` → MarkdownV2. Example on its own line:\n"
    ">    `**Что зафиксировано**`\n"
    "> 2. **Inside bullet or numbered list items** — DO NOT use `**bold**`. "
    "Telegram MarkdownV2 conversion is buggy for inline list emphasis and "
    "double-asterisks leak as literal `**`. Instead use ALL CAPS or trailing colon:\n"
    ">    `• PHASE 1`  or  `• Phase 1:`  (NOT `• **Phase 1**`)\n"
    "> 3. **Inside `clarify_tool` `choices` array** — PLAIN TEXT only. No `**bold**`, "
    "no `__underline__`. The inline-keyboard render path does NOT apply MarkdownV2 "
    "conversion.\n"
    "> Additional emphasis tools: «кавычки-ёлочки» for quotes, `→` prefix for lists. "
    "Single `*italic*` and inline `\\`code\\`` are fine everywhere.\n"
    ">\n"
