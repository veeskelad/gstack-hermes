...
metadata:
  hermes:
    tags: [yc, review, scope, planning]
    related_skills: [yc-eng-review, yc-design-review]
    pipeline_successor: writing-plans
---
...
> Let gstack proactively suggest skills, like /qa for "does this work?" or /yc-investigate for bugs?
...
Key routing rules:
- Product ideas/brainstorming → invoke /yc-office-hours
- Strategy/scope → invoke /yc-ceo-review
- Architecture → invoke /yc-eng-review
- Design system/plan review → invoke /design-consultation or /yc-design-review
- Full review pipeline → invoke /autoplan
- Bugs/errors → invoke /yc-investigate
- QA/testing site behavior → invoke /qa or /qa-only
- Code review/diff check → invoke /requesting-code-review
...
Skills that run plan reviews (`/plan-*-review`, `/codex review`) include the EXIT PLAN MODE GATE blocking checklist at the end of the skill, which verifies the plan file ends with `## GSTACK REVIEW REPORT` before ExitPlanMode is called. Skills that don't run plan reviews (operational skills like `/ship`, `/qa`, `/requesting-code-review`) typically don't operate in plan mode and have no review report to verify; this footer is a no-op for them. Writing the plan file is the one edit allowed in plan mode.
...
SLUG=$(~/.claude/skills/gstack/browse/bin/remote-slug 2>/dev/null || basename "${HERMES_PROJECT:-$(pwd)}")
BRANCH=$(git rev-parse --abbrev-ref HEAD 2>/dev/null | tr '/' '-' || echo 'no-branch')
DESIGN=$(ls -t ~/.hermes/projects/$SLUG/*-$BRANCH-design-*.md 2>/dev/null | head -1)
[ -z "$DESIGN" ] && DESIGN=$(ls -t ~/.hermes/projects/$SLUG/*-design-*.md 2>/dev/null | head -1)
[ -n "$DESIGN" ] && echo "Design doc found: $DESIGN" || echo "No design doc found"
```
If a design doc exists (from `/yc-office-hours`), read it. Use it as the source of truth for the problem statement, constraints, and chosen approach. If it has a `Supersedes:` field, note that this is a revised design.

**Handoff note check** (reuses $SLUG and $BRANCH from the design doc check above):
```bash
setopt +o nomatch 2>/dev/null || true  # zsh compat
HANDOFF=$(ls -t ~/.hermes/projects/$SLUG/*-$BRANCH-ceo-handoff-*.md 2>/dev/null | head -1)
...
from a prior CEO review session that paused so the user could run `/yc-office-hours`. Use it
...
> "No design doc found for this branch. `/yc-office-hours` produces a structured problem
> statement, premise challenge, and explored alternatives — it gives this review much
> sharper input to work with. Takes about 10 minutes. The design doc is per-feature,
> not per-product — it captures the thinking behind this specific change."

Options:
- A) Run /yc-office-hours now (we'll pick up the review right after)
- B) Skip — proceed with standard review

If they skip: "No worries — standard review. If you ever want sharper input, try
/yc-office-hours first next time." Then proceed normally. Do not re-offer later in the session.

If they choose A:

Say: "Running /yc-office-hours inline. Once the design doc is ready, I'll pick up
the review right where we left off."

Read the `/yc-office-hours` skill file at `~/.claude/skills/gstack/yc-office-hours/SKILL.md` using the Read tool.

**If unreadable:** Skip with "Could not load /yc-office-hours — skipping." and continue.
...
After /yc-office-hours completes, re-run the design doc check:
```bash
setopt +o nomatch 2>/dev/null || true  # zsh compat
SLUG=$(~/.claude/skills/gstack/browse/bin/remote-slug 2>/dev/null || basename "${HERMES_PROJECT:-$(pwd)}")
BRANCH=$(git rev-parse --abbrev-ref HEAD 2>/dev/null | tr '/' '-' || echo 'no-branch')
...
sure," or is clearly exploring rather than reviewing — offer `/yc-office-hours`:

> "It sounds like you're still figuring out what to build — that's totally fine, but
> that's what /yc-office-hours is designed for. Want to run /yc-office-hours right now?
> We'll pick up right where we left off."

Options: A) Yes, run /yc-office-hours now. B) No, keep going.
...
|------|-------------------|
| running the 11-section deep review, required outputs, and review report (only after Step 0 scope and mode are agreed) | `sections/requesting-code-review-sections.md` |
...
```bash
eval "$(~/.claude/skills/gstack/bin/gstack-slug 2>/dev/null)" && mkdir -p ~/.hermes/projects/$SLUG/ceo-plans
```

Before writing, check for existing CEO plans in the ceo-plans/ directory. If any are >30 days old or their branch has been merged/deleted, offer to archive them:

```bash
mkdir -p ~/.hermes/projects/$SLUG/ceo-plans/archive
# For each stale plan: mv ~/.hermes/projects/$SLUG/ceo-plans/{old-plan}.md ~/.hermes/projects/$SLUG/ceo-plans/archive/
```

Write to `~/.hermes/projects/$SLUG/ceo-plans/{date}-{feature-slug}.md` using this format:
...
Generated by /yc-ceo-review on {date}
...
> **STOP.** Before running the 11-section deep review, required outputs, and review report (only after Step 0 scope and mode are agreed), Read `~/.claude/skills/gstack/yc-ceo-review/sections/requesting-code-review-sections.md` and execute it
> in full. Do not work from memory — that section is the source of truth for this step.

## Section self-check (before you finish)

You ran a carved skill. The Section index above named `sections/requesting-code-review-sections.md`
