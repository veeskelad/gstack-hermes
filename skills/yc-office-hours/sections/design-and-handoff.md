...
PRIOR=$(ls -t ~/.hermes/projects/$SLUG/*-$BRANCH-design-*.md 2>/dev/null | head -1)
```
If `$PRIOR` exists, the new doc gets a `Supersedes:` field referencing it. This creates a revision chain — you can trace how a design evolved across office hours sessions.

Write to `~/.hermes/projects/{slug}/{user}-{branch}-design-{datetime}.md`.

After writing the design doc, tell the user:
**"Design doc saved to: {full path}. Other skills (/yc-ceo-review, /yc-eng-review) will find it automatically."**
...
Generated by /yc-office-hours on {date}
Branch: {branch}
Repo: {owner/repo}
Status: DRAFT
Mode: Startup
...
- **`/yc-ceo-review`** for ambitious features (EXPANSION mode) — rethink the problem, find the 10-star product
- **`/yc-eng-review`** for well-scoped implementation planning — lock in architecture, tests, edge cases
- **`/yc-design-review`** for visual/UX design review

The design doc at `~/.hermes/projects/` is automatically discoverable by downstream skills — they will read it during their pre-review system audit.
