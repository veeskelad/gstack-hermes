...
```bash
eval "$(~/.claude/skills/gstack/bin/gstack-slug 2>/dev/null)" && mkdir -p ~/.hermes/projects/$SLUG
USER=$(whoami)
DATETIME=$(date +%Y%m%d-%H%M%S)
```

Write to `~/.hermes/projects/{slug}/{user}-{branch}-eng-review-test-plan-{datetime}.md`:

```markdown
# Test Plan
Generated by /yc-eng-review on {date}
Branch: {branch}
Repo: {owner/repo}
...
### TODOS.md updates
After all review sections are complete, present each potential TODO as its own individual AskUserQuestion. Never batch TODOs — one per question. Never silently skip this step. Follow the format in `.claude/skills/requesting-code-review/TODOS-format.md`.
...
- **Outside Voice (optional):** Independent plan review from a different AI model. Offered after all review sections complete in /yc-ceo-review and /yc-eng-review. Falls back to Claude subagent if Codex is unavailable. Never gates shipping.
...
|--------|---------|-----|------|--------|----------|
| CEO Review | \`/yc-ceo-review\` | Scope & strategy | {runs} | {status} | {findings} |
| Codex Review | \`/codex review\` | Independent 2nd opinion | {runs} | {status} | {findings} |
| Eng Review | \`/yc-eng-review\` | Architecture & tests (required) | {runs} | {status} | {findings} |
| Design Review | \`/yc-design-review\` | UI/UX gaps | {runs} | {status} | {findings} |
| DX Review | \`/yc-devex-review\` | Developer experience gaps | {runs} | {status} | {findings} |
\`\`\`
...
**Suggest /yc-design-review if UI changes exist and no design review has been run** — detect from the test diagram, architecture review, or any section that touched frontend components, CSS, views, or user-facing interaction flows. If an existing design review's commit hash shows it predates significant changes found in this eng review, note that it may be stale.

**Mention /yc-ceo-review if this is a significant product change and no CEO review exists** — this is a soft suggestion, not a push. CEO review is optional. Only mention it if the plan introduces new user-facing features, changes product direction, or expands scope substantially.
...
- **A)** Run /yc-design-review (only if UI scope detected and no design review exists)
- **B)** Run /yc-ceo-review (only if significant product change and no CEO review exists)
