...
metadata:
  hermes:
    tags: [yc, retrospective, learnings]
    related_skills: [yc-learn]
    pipeline_successor: yc-learn
---
...
## User-invocable
When the user types `/yc-retro`, run this skill.

## Arguments
- `/yc-retro` — default: last 7 days
- `/yc-retro 24h` — last 24 hours
- `/yc-retro 14d` — last 14 days
- `/yc-retro 30d` — last 30 days
- `/yc-retro compare` — compare current window vs prior same-length window
- `/yc-retro compare 14d` — compare with explicit window
- `/yc-retro global` — cross-project retro across all AI coding tools (7d default)
- `/yc-retro global 14d` — cross-project retro with explicit window
...
```
Usage: /yc-retro [window | compare | global]
  /yc-retro              — last 7 days (default)
  /yc-retro 24h          — last 24 hours
  /yc-retro 14d          — last 14 days
  /yc-retro 30d          — last 30 days
  /yc-retro compare      — compare this period vs prior period
  /yc-retro compare 14d  — compare with explicit window
  /yc-retro global       — cross-project retro across all AI tools (7d default)
  /yc-retro global 14d   — cross-project retro with explicit window
```
...
```bash
[ -f ~/.gstack/yc-retro-context.md ] && echo "RETRO_CONTEXT_FOUND" || echo "NO_RETRO_CONTEXT"
```

If `RETRO_CONTEXT_FOUND`: read `~/.gstack/yc-retro-context.md`. This file is user-authored and may contain meeting notes, calendar events, decisions, and other context that doesn't appear in git history. Incorporate this context into the retro narrative where relevant.
...
```
| Skill Usage | /ship(12) /qa(8) /requesting-code-review(5) · 3 safety hook fires |
```
...
```
  EUREKA /yc-office-hours (branch: garrytan/auth-rethink): "Session tokens don't need server storage — browser crypto API makes client-side JWT validation viable"
  EUREKA /yc-eng-review (branch: garrytan/cache-layer): "Redis isn't needed here — Bun's built-in LRU cache handles this workload"
```
...
setopt +o nomatch 2>/dev/null || true  # zsh compat
eval "$(~/.claude/skills/gstack/bin/gstack-slug 2>/dev/null)"
cat ~/.hermes/projects/$SLUG/*-reviews.jsonl 2>/dev/null | grep '"skill":"ship"' | grep '"plan_items_total"' || echo "NO_PLAN_DATA"
```
...
When the user runs `/yc-retro global` (or `/yc-retro global 14d`), follow this flow instead of the repo-scoped Steps 1-14. This mode works from any directory — it does NOT require being inside a git repo.
...
If `total_sessions` is 0, say: "No AI coding sessions found in the last <window>. Try a longer window: `/yc-retro global 30d`" and stop.
...
- Top Work: 3 bullet points summarizing the user's major themes, inferred from
  ...
  E.g., "Built /yc-retro global — cross-project retrospective with AI session discovery"
  not "feat: gstack-global-discover" + "feat: /yc-retro global template".
...
When the user runs `/yc-retro compare` (or `/yc-retro compare 14d`):
