...
# /sec-cso — Chief Security Officer Audit (v2)
...
## User-invocable
When the user types `/sec-cso`, run this skill.

## Arguments
- `/sec-cso` — full daily audit (all phases, 8/10 confidence gate)
- `/sec-cso --comprehensive` — monthly deep scan (all phases, 2/10 bar — surfaces more)
- `/sec-cso --infra` — infrastructure-only (Phases 0-6, 12-14)
- `/sec-cso --code` — code-only (Phases 0-1, 7, 9-11, 12-14)
- `/sec-cso --skills` — skill supply chain only (Phases 0, 8, 12-14)
- `/sec-cso --diff` — branch changes only (combinable with any above)
- `/sec-cso --supply-chain` — dependency audit only (Phases 0, 3, 12-14)
- `/sec-cso --owasp` — OWASP Top 10 only (Phases 0, 9, 12-14)
- `/sec-cso --scope auth` — focused audit on a specific domain
...
**Daily mode (default, `/sec-cso`):** 8/10 confidence gate. Zero noise. Only report what you're sure about.
...
**Comprehensive mode (`/sec-cso --comprehensive`):** 2/10 confidence gate. Filter true noise only (test fixtures, documentation, placeholders) but include anything that MIGHT be a real issue. Flag these as `TENTATIVE` to distinguish from confirmed findings.
...
**This tool is not a substitute for a professional security audit.** /sec-cso is an AI-assisted
...
sensitive data, payments, or PII, engage a professional penetration testing firm. Use /sec-cso as
...
**Always include this disclaimer at the end of every /sec-cso report output.**
